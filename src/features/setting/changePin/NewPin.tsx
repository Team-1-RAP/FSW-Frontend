...
export const NewPin = () => {
  ...
  return (
    <FormResetPasswordPinTemplate title="Buat Pin" message="Buat pin baru yang belum pernah digunakan dan sesuai dengan ketentuan">
      <form action="submit" onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4 w-full">
              ...
              render={({ field }) => (
                <>
                  ...
                  <input
                    ...
                    {...field}
                    className={`w-full h-12 border-[1px] rounded-[10px] px-4 ${errors.pin ? "input-error" : "border-[#A09FA4]"}`}
                    ...
                    placeholder="Pin"
                  />
                  <button type="button" className={`absolute right-2 ${errors.pin ? "text-red-500" : "text-gray-500"}`} onClick={setShowPin}>
                    {showPin ? <Eye /> : <EyeOff />}
                  </button>
                </>
              )}
              ...
              render={({ field }) => (
                <>
                  ...
                  <input
                    ...
                    {...field}
                    className={`w-full h-12 border-[1px] rounded-[10px] px-4 ${errors.confirmPin ? "input-error" : "border-[#A09FA4]"}`}
                    onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
                      const value = event.target.value
                      if (/^[0-9]*$/.test(value) && value.length <= 6) {
                        field.onChange(value)
                  ...
                  />
                  <button type="button" className={`absolute right-2 ${errors.confirmPin ? "text-red-500" : "text-gray-500"}`} onClick={setShowConfirmPin}>
        ...
        <button type="submit" className="bg-[#0066AE] h-12 rounded-[10px] text-white hover:bg-sky-900 focus:bg-sky-950 px-16" onClick={(e) => e.currentTarget}>
          Selanjutnya
        </button>
      </form>
