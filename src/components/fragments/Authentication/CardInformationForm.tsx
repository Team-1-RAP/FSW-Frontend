...
export const CardInformationForm = ({
  onSubmit,
  errorMessage,
  title,
  titleIsCenter = true,
  ...
  return (
    <FormResetPasswordPinTemplate title={title} titleIsCenter={titleIsCenter}>
      <div className="flex flex-col gap-3 items-center pb-8">
        <Card variant="purpleCyan" size="sm" previewHidden={true} />
        {/* Error */}
        {errorMessage && (
          <div className="text-red-500 mt-4">Pastikan Data Benar</div>
        )}
        <form ref={formRef} className="flex flex-col">
          <p className="text-start mb-3">Masukan Nomor Kartu:</p>
          <Controller
            name="cardNumber"
            control={control}
            render={({ field }) => (
              <div className={`${!errors.cardNumber && "mb-3"}`}>
                ...
                <input
                  ...
                  className={`w-full h-12 border-[1px] rounded-[10px] px-4 ${
                    errors.cardNumber ? "input-error" : "border-[#A09FA4]"
                  }`}
              ...
              render={({ field }) => (
                <div className="w-full">
                  ...
                  <input
                    ...
                    placeholder="MM"
                    className={`w-full h-12 border-[1px] rounded-[10px] px-4 ${
                      errors.cardExpMonth ? "input-error" : "border-[#A09FA4]"
                    }`}
                    onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
                      const value = event.target.value;
              ...
              render={({ field }) => (
                <div className="w-full">
                  ...
                  <input
                    ...
                    placeholder="YYYY"
                    className={`w-full h-12 border-[1px] rounded-[10px] px-4 ${
                      errors.cardExpYear ? "input-error" : "border-[#A09FA4]"
                    }`}
                    onChange={(event: React.ChangeEvent<HTMLInputElement>) => {
                      const value = event.target.value;
