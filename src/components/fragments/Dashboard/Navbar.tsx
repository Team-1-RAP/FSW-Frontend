...
const Navbar: React.FC<NavbarProps> = ({ toggleAside }) => {
  ...
  return (
    <header
      className="h-25 lg:h-20 bg-[#FFFFFF] top-0 w-full py-5 lg:py-0 fixed shadow z-10"
      role="banner"
    >
      <div className="flex flex-col justify-between h-full px-4 mb-auto lg:flex-row lg:items-center gap-y-3 md-gap-y-0 lg:px-10">
        ...
        <div className="flex flex-row">
          <button
            className="lg:hidden flex items-center justify-center bg-[#F1F5F9] w-[36px] h-[36px] rounded-full"
            onClick={toggleAside}
            ...
            <div className="relative">
              ...
              {isDropdownOpen && filteredServices.length > 0 && (
                <ul
                  ...
                  className="absolute z-20 mt-2 w-full bg-white border border-[#F1F5F9] rounded-2xl shadow-md"
                  role="listbox"
                ...
                >
                  {filteredServices.map((service) => (
                    <li
                      key={service.id}
                      className="cursor-pointer px-4 py-2 hover:bg-[#F1F5F9] rounded-2xl"
                      role="option"
                      onClick={() => handleServiceSelect(service.link)}
                    >
                      {service.label}
                    </li>
                  ))}
          ...
          <Link
            to="/pusat-bantuan"
            className="flex items-center justify-center bg-[#F1F5F9] w-[36px] h-[36px] rounded-full"
