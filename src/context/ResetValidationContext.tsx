...
export const ResetValidationProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [cardNumber, setCardNumber] = useState<IAccount | null>(null)
  const [email, setEmail] = useState<string | null>(null)

  const validationCard = async (cardNumber: string, cardExpMonth: string, cardExpYear: string) => {
    try {
      const response = await fetch(import.meta.env.VITE_API_BASE_URL_NON_TRANSACTION + "reset/password/validation/card", {
        method: "POST",
        headers: {
  ...
  const validationBirthDate = async (atm_card_no: string, birthDate: string) => {
    try {
      const response = await fetch(import.meta.env.VITE_API_BASE_URL_NON_TRANSACTION + "reset/password/validation/birthDate", {
        method: "POST",
        headers: {
          accept: "application/json",
          "Content-Type": "application/json",
  ...
  const validationEmail = async (atm_card_no: string, email: string) => {
    try {
      const response = await fetch(import.meta.env.VITE_API_BASE_URL_NON_TRANSACTION + "reset/password/validation/email", {
        method: "POST",
        headers: {
          accept: "application/json",
          "Content-Type": "application/json",
  ...
  const validationOtp = async (atm_card_no: string, otp: string) => {
    try {
      const response = await fetch(import.meta.env.VITE_API_BASE_URL_NON_TRANSACTION + "reset/password/validation/otpVerify", {
        method: "POST",
        headers: {
          accept: "application/json",
          "Content-Type": "application/json",
  ...
  const resetPin = async (atm_card_no: string, pin: string, confirmPin: string) => {
    try {
      const response = await fetch(import.meta.env.VITE_API_BASE_URL_NON_TRANSACTION + "reset/password", {
        method: "POST",
        headers: {
          accept: "application/json",
          "Content-Type": "application/json",
